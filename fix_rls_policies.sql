...
-- Create a more permissive policy that allows registration
-- Allow authenticated users to insert new records (for registration)
-- Allow users to read/update their own data
CREATE POLICY users_registration_and_own_data ON users
...
-- Also fix the helper functions to handle the case where user doesn't exist yet
...
-- Keep the existing readable policy for donors
CREATE POLICY donors_readable_by_patients ON donors
FOR SELECT TO authenticated
USING (is_patient() OR is_admin());

-- Success message
SELECT 'RLS policies updated successfully for user registration!' as message;